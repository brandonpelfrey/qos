...
void cr3_write(u64 newval)
{
  asm volatile("mov %0, %%cr3" : : "r"(newval) : "memory");
}
...
void parse_physical_memory(SMAP_entry* smap_data)
{
  ...
  size_t region_size =
    ((size_t)__conventional_memory_root->LengthH << 32) | (size_t)__conventional_memory_root->LengthL;
  auto region_size_kib = region_size / 1024;
  kprintf(VM,
          "Found %d.%d MiB usable RAM @ Physical address 0x%08x%08x\n",
          region_size_kib / 1024,
          region_size_kib % 1024,
          __conventional_memory_root->BaseH,
          __conventional_memory_root->BaseL);
}
